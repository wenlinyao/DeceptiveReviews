...
!!! RUN this program:
go to run folder, 
THEANO_FLAGS=mode=FAST_RUN,device=cpu,floatX=float32 python ../CNN/run_main_CNN.py 
!!!
...
if __name__ == '__main__':
    ...
    for valid_train_cat in cate_setList:
        ...
        for i, valid_test_cat in enumerate(cate_setList):
            ...
            print "conv_net_sentence_main(...)"
            conv_net_sentence_main(current_folder, repeat_num)
            
            for rand_seed in range(0, repeat_num):
                dst_file = 'thread' + str(rand_seed) + '/CNN_true_and_pred_value'
